...
SUITE(EncryptTest)
{
    TEST_FIXTURE(KeyBFixture, UpperCaseString) {
        CHECK_WIDE_EQUAL(L"ГТЁНРСЙГЁУ", cipher->encrypt(L"ВСЕМПРИВЕТ"));
    }
    ...
    TEST_FIXTURE(KeyBFixture, StringWithWhitespaceAndPunctuation) {
        CHECK_WIDE_EQUAL(L"РСЙГЁУЕСФД", cipher->encrypt(L"Привет, друг"));
    }
    
    TEST_FIXTURE(KeyBFixture, StringWithNumbers) {
        CHECK_WIDE_EQUAL(L"ТОПГЬНДПЕПН", cipher->encrypt(L"С Новым 2025 Годом"));
    }
    ...
    TEST(MaxShiftKey) {
        ...
        CHECK_WIDE_EQUAL(L"БРДЛОПЗБДС", cipher.encrypt(L"ВСЕМПРИВЕТ"));
    }
}
...
{
    TEST_FIXTURE(KeyBFixture, UpperCaseString) {
        CHECK_WIDE_EQUAL(L"ВСЕМПРИВЕТ", cipher->decrypt(L"ГТЁНРСЙГЁУ"));
    }
    
    TEST_FIXTURE(KeyBFixture, LowerCaseString) {
        CHECK_THROW(cipher->decrypt(L"гтёнрсйГЁУ"), cipher_error);
    }
    
    TEST_FIXTURE(KeyBFixture, WhitespaceString) {
        CHECK_THROW(cipher->decrypt(L"ГТЁ НРС ЙГЁУ"), cipher_error);
    }
    
    TEST_FIXTURE(KeyBFixture, DigitsString) {
        CHECK_THROW(cipher->decrypt(L"ТОПГЬНДПЕПН2025"), cipher_error);
    }
    ...
    TEST(MaxShiftKey) {
        modAlphaCipher cipher(L"Я");
        CHECK_WIDE_EQUAL(L"ВСЕМПРИВЕТ", cipher.decrypt(L"БРДЛОПЗБДС"));
    }
}
